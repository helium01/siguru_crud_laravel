@include('tmp.headercss')
    <title>data kelurahan || admin</title>
        <!-- ============================================================== -->
        <!-- End Topbar header -->
        <!-- ============================================================== -->
        <!-- ============================================================== -->
        ...
        <div class="page-wrapper">
            ...
            <div class="white-box col-md-12 col-lg-12 col-sm-12">
                            <div class="d-md-flex mb-3">
                                <h3 class="box-title mb-0">Data Kelurahan</h3>
                            ...
                            <div class="table-responsive">
                                <table class="table no-wrap">
                                    ...
                                    @foreach($kelurahan as $k)
                                        <tr>
                                            <td>{{$k->id_kelurahan}}</td>
                                            <td class="txt-oflo">{{$k->nama_kelurahan}}</td>
                                            <td class="txt-oflo">{{$k->id_kecamatan}}</td>
                                            <td><span class="text-success">
                                            <a class="btn btn-primary" href="/kelurahan/edit/{{$k->id_kelurahan}}">edit</a>
                                            <a class="btn btn-danger" href="/kelurahan/del/{{$k->id_kelurahan}}">hapus</a>
                                            </span></td>
                                        </tr>
                                    @endforeach 
                                    </tbody>
            ...
            <form action="/kelurahan/upload" method="post">
            {{csrf_field()}}
                <div class="form-group">
                    <label for="exampleInputEmail1">Nama Kelurahan</label>
                    <input name="nama_kelurahan" type="text"  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">id_kecamatan</label>
                    <select name="id_kecamatan" class="form-control" id="exampleFormControlSelect1">
                    @foreach($kecamatan as $kc)
                    <option value="{{$kc->id_kecamatan}}">{{$kc->id_kecamatan}} || {{$kc->nama_kecamatan}}</option>
                    @endforeach
                    </select>
                </div>
